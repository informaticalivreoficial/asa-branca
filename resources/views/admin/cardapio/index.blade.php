...
<div class="row mb-2">
    <div class="col-sm-6">
        <h1><i class="fas fa-search mr-2"></i> Gerenciar Cardápio</h1>
    </div>
    <div class="col-sm-6">
...
@section('content')
    <div class="card">
        <div class="card-header text-right">
            <a href="{{route('cardapio.create')}}" class="btn btn-default"><i class="fas fa-plus mr-2"></i> Cadastrar Novo</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
            ...
            @if(!empty($itens) && $itens->count() > 0)
                <table id="example1" class="table table-bordered table-striped projects">
                    ...
                    <tbody> 
                        ...
                        <tr style="{{ ( $item->status == '1' ? '' : 'background: #fffed8 !important;')  }}">                            
                            <td class="text-center">
                                <a href="{{  $item->cover() }}" data-title="{{ $item->titulo}}" data-toggle="lightbox">
                                    <img width="60" alt="{{ $item->titulo}}" src="{{ $item->cover()}}">
                                </a>
                            </td>
                            <td>{{$item->titulo}}</td>
                            <td>{{$item->created_at}}</td>
                            <td class="text-center">{{$item->views}}</td>
                            <td class="text-center">{{$item->countimages()}}</td>                            
                            <td>
                                ...
                                <a href="{{ route('cardapio.edit', [ 'id' => $item->id ]) }}" class="btn btn-xs btn-default"><i class="fas fa-pen"></i></a>
                                <button type="button" class="btn btn-xs btn-danger text-white j_modal_btn" data-id="{{$item->id}}" data-toggle="modal" data-target="#modal-default"><i class="fas fa-trash"></i></button>
                            </td>
                        </tr>                            
                        @endforeach
                    </tbody>                
